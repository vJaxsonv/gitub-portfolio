Hello my name is Jaxson DeHaven and this is my portfolio of all the projects I have created from Cincinnati State. 




1. AIRPLANE  APPLICATION
        ##Setup
   - Unzip the file and download the https://github.com/vJaxsonv/gitub-portfolio/blob/main/AirplaneAttendantsDatabase.sql
   - Open the file through Microsoft SQl
   - Create a database called dbFlyMe2theMoon and execute the database

                
<img width="1155" height="718" alt="image" src="https://github.com/user-attachments/assets/a25a6327-f7c0-46e7-92b2-90d58a93c155" />


- After Download https://github.com/vJaxsonv/gitub-portfolio/blob/main/AirplaneApp.zip and extract to which ever folder you would like
- Open Visual Studio
- Now you will be able to run the Program!

  <img width="2532" height="1352" alt="image" src="https://github.com/user-attachments/assets/fe2e95e5-6681-43d6-b29c-26e4cfe0d944" />

2. CENSUSBUREAU APPLICATION
        ##Setup
   - Unzip the file and download the https://github.com/vJaxsonv/gitub-portfolio/blob/main/CensusBureau.zip and open in visual studio then hit start
        <img width="1859" height="997" alt="image" src="https://github.com/user-attachments/assets/b2cf4fe9-65a9-40d9-9ab6-ba2b29828434" />

4. PAYMENT DEDUCTION APPLICATION
        ##Setup
   - Unzip the file and download the https://github.com/vJaxsonv/gitub-portfolio/blob/main/PaymentDeductionApplication.zip
   - Open in visual studio then hit start
           <img width="2505" height="1349" alt="image" src="https://github.com/user-attachments/assets/16ea5933-46d4-46e4-92c6-914a892b189f" />
5. PLUMBING DATABASE
        ##Setup
   - Download the .sql file https://github.com/vJaxsonv/gitub-portfolio/blob/main/PlumbingDatabase.sql
   - Open Micrsoft Sql
   - create a database called dbFixinyerleaks and execute the database
   - And Press Execute
           <img width="1430" height="874" alt="image" src="https://github.com/user-attachments/assets/f8d04fd6-b38a-4c4b-9698-78a5c7b66af1" />
6. SUPER STRING
        ##Setup
   - Download https://github.com/vJaxsonv/gitub-portfolio/blob/main/SuperString.zip and unzlp
   - Open the SuperString file and double click SuperString.sln (Mine says Final Project but its the same solution)
    <img width="1433" height="760" alt="image" src="https://github.com/user-attachments/assets/fb293387-90d1-406e-830c-02377fa2b354" />
    <img width="2460" height="1299" alt="image" src="https://github.com/user-attachments/assets/4cfecce0-0ebd-4dff-ac91-ba33f220151b" />
7. XML Student Processor
   

   




